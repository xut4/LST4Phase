...
from sinc_branch import SincFrequencyBranch
# =========================
# SeismoDual-style TF branch
# =========================
class SeismoDualLFBBranch(nn.Module):
    """
    Sinc-based learnable frequency branch
    input:  (B, 3, T)
    output: (B, out_ch, target_len)
    """
    def __init__(self, out_ch, sample_rate=100, sinc_filters=12, sinc_kernel=129):
        super().__init__()
        self.branch = SincFrequencyBranch(
            out_ch=out_ch,
    ...
    def forward(self, x, target_len):
        feat = self.branch(x, target_len=target_len)   # (B, L, out_ch)
...
class ModelLFB(nn.Module):
    def __init__(self, 
        ...
        self.tf_branch = SeismoDualLFBBranch(
            out_ch=self.ch1,
            sample_rate=100,  ##
            sinc_filters=12,
            sinc_kernel=129
