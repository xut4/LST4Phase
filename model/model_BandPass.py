...
# =========================
# Fixed bandpass branch
# =========================
def design_bandpass_sinc(low_hz, high_hz, kernel_size, sample_rate):
    if kernel_size % 2 == 0:
        kernel_size += 1
    ...
    h_high = 2 * high_hz * np.sinc(2 * high_hz * t)
    h_low = 2 * low_hz * np.sinc(2 * low_hz * t)
    h = h_high - h_low

    window = np.hamming(kernel_size)
    h = h * window

    h = h / (np.sum(np.abs(h)) + 1e-8)
    return h.astype(np.float32)


class FixedBandpassConv(nn.Module):
    def __init__(self, bands, kernel_size=129, sample_rate=100, stride=1, padding=None):
        super().__init__()
        self.bands = bands
        ...
        for low, high in bands:
            h = design_bandpass_sinc(low, high, kernel_size, sample_rate)
            filters.append(h)

        filters = np.stack(filters, axis=0)   # (K, L)
        filters = torch.tensor(filters).unsqueeze(1)  # (K, 1, L)
...
class FixedBandpassBranch(nn.Module):
    """
    raw waveform -> RSS -> fixed bandpass bank -> abs/log compression
    -> conv -> projection
    output: (B, out_ch, target_len)
    """
    def __init__(self, out_ch, sample_rate=100, kernel_size=129, use_log_compression=True):
        super().__init__()
        ...
        in_filters = len(self.bands)

        self.tf_conv = nn.Sequential(
            nn.Upsample(scale_factor=1.5),
            nn.Conv1d(in_filters, 16, kernel_size=5, padding="same"),
            nn.ReLU(),
            nn.Upsample(scale_factor=1.5),
    ...
    def forward(self, x, target_len):
        # x: (B, 3, T)
        rss = torch.sqrt(torch.clamp(torch.sum(x ** 2, dim=1, keepdim=True), min=1e-8))
        feat = self.bp(rss)   # (B, num_bands, T)
        ...
        feat = feat.permute(0, 2, 1)   # (B, out_ch, L)
        return feat
...
class ModelBP(nn.Module):
    def __init__(self, 
        ...
        self.tf_branch = FixedBandpassBranch(
            out_ch=self.ch1,
            sample_rate=100,
            kernel_size=129,
    ...
    def forward(self, x):
        ...
        tf1 = self.tf_branch(x, target_len=x1.shape[-1])  # (B,16,L1)
        x1 = self.fusion1(x1, tf1)
...
if __name__ == '__main__':
    ...
    model = ModelBP(    in_length=100*30,    in_channels=3,    class_num=3,    strides=[3,2,2],    kernel_size=3).to('cpu')
